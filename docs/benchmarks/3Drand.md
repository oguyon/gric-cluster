...
| **Execution Time** | `4437.228 ms` | Total wall-clock runtime |
| **Throughput** | `4,507 fps` | Frames processed per second |
| **Active Clusters / States ($K$)** | `368` | Total distinct clusters created |
| **Sample Distances ($d_S$)** | `101,787` | Sample-to-cluster evaluations |
| **Search Calls ($d_S$ / frame)** | **`5.09`** | Search calls per frame |
| **Total Ops ($d$ / frame)** | **`8.47`** | Total distance ops per frame |
| **Pruning Speedup Factor** | **`72.3x`** | Acceleration over exhaustive search ($K / d_S$) |
