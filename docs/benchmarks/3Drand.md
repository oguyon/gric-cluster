...
| **Execution Time** | `3923.430 ms` | Total wall-clock runtime |
| **Throughput** | `5,097 fps` | Frames processed per second |
| **Active Clusters / States ($K$)** | `373` | Total distinct clusters created |
| **Sample Distances ($d_S$)** | `101,674` | Sample-to-cluster evaluations |
| **$d_S / \text{frame}$** | `**5.08**` | Search calls per frame |
| **Total $d / \text{frame}$** | `**8.55**` | Total distance ops per frame |
| **Pruning Speedup Factor** | `**73.4x**` | Acceleration over exhaustive search ($K / d_S$) |
