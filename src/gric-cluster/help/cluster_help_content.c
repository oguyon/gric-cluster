 ...
 * @brief Help content database implementations.
 */
    ...
    const char *key)
{
    ...
    else if (strcmp(key, "tiling") == 0)
    {
        print_help_section(
            ...
            "1. Arithmetic Speedup: Distance computations check\n"
            "   smaller sub-frames (e.g. a 2x2 tile distance\n"
            "   call is 4x cheaper). Per-tile search complexity\n"
            "   drops from O(K) to O(K_tile).\n"
            "2. Parallelization: OpenMP dispatches one task per\n"
            "   tile, running them concurrently.\n"
            "3. Memory Reduction: Anchors store only tile pixels\n"
            "   (e.g. 1/4 the size for 2x2). Simpler local\n"
            "   environments mean fewer clusters per tile\n"
            "   (K_tile << K), reducing anchor storage by\n"
            "   10-100x and DCC matrices by 100-1000x.");
        print_help_section(
            "TWO-PASS PIPELINE",
            ...
            "  sub-frames (pixel extraction, not interpolation).\n"
            "  Each tile independently runs the standard GRIC\n"
            "  clustering pipeline (predict priors, select\n"
            "  target, measure distance, update & prune, assign).\n"
            "  All tile tasks execute in parallel via OpenMP.\n"
            "  Result: one cluster assignment per tile.\n"
            "\n\n"
            "Pass 2: Joint Trajectory Fusion (JTF)\n"
            "  After all tiles complete Pass 1, JTF corrects\n"
            "  tile-boundary noise by leveraging cross-tile\n"
            "  correlations from recent history.\n"
            "\n"
            "  For each tile, JTF:\n"
            "  1. Builds a spatial key from the current Pass 1\n"
            "     assignments of all tiles.\n"
            "  2. Builds a temporal key from the previous frame's\n"
            "     assignment tuple.\n"
            "  3. Scans the last -retrieval_window tuples for\n"
            "     matching spatial+temporal patterns.\n"
            "  4. Multiplies the Pass 1 posterior by the match\n"
            "     scores. If the fused argmax differs from the\n"
            "     Pass 1 assignment, overrides it.\n"
            "\n"
            "  JTF never overrides a newly created cluster\n"
            "  (Pass 1 produced a new anchor). It only refines\n"
        ...
        print_help_section(
            "TILING OPTIONS",
            "-tiles <NxM>\n"
            "  Grid size. N columns, M rows.\n"
            "  Example: -tiles 2x2 creates 4 tiles.\n"
            "\n\n"
            "-tileconf <file.txt>\n"
            "  Per-tile configuration overrides.\n"
            "  ASCII file, one line per tile, 3 space-separated\n"
            "  fields: tile_id rlim maxnbclust.\n"
            "  Lines starting with '#' are comments.\n"
            "  Example: '0 0.8 500' sets tile 0 to rlim=0.8\n"
            "  with maxnbclust=500.\n"
            "\n\n"
            "-retrieval_window <N>\n"
            "  Number of recent frames to scan during JTF\n"
            "  pattern matching (default: 1000). Larger values\n"
            "  improve accuracy but increase scan cost linearly.");
        print_help_section(
            "OPTION INTERACTIONS",
            "All per-tile options (-gprob, -entropy,\n"
            "-soft_bayesian, -sparse_dcc, -te4/-te5) work\n"
            "independently within each tile during Pass 1.\n"
            "Each tile has its own ClusterState, DCC matrix,\n"
            "visitor history, and entropy scheduler.\n"
            "\n\n"
            "-pred (sequence prediction)\n"
            "  Operates at the joint tuple level before Pass 1.\n"
            "  predict_joint_tuples() scans tuple history\n"
            "  for matching multi-tile patterns, then populates\n"
            "  mixed priors and candidate lists for all tiles.\n"
            "\n\n"
            "-tm (transition matrix)\n"
            "  Operates independently per tile during Pass 1.\n"
            "\n\n"
            "-ncpu <N>\n"
            "  Tiles run as OpenMP tasks. When tiles < cpus,\n"
            "  nested parallelism is enabled so each tile can\n"
            "  use intra-tile parallelism (e.g. in framedist).");
        print_help_section(
            "TUNING GUIDELINES",
            "1. Grid Resolution:\n"
            "   Avoid partitioning too finely (e.g. 4x4 on a\n"
            "   32x32 image). As grid size M increases, joint\n"
            "   state combinations grow as k^M, causing\n"
            "   combinatorial explosion. OpenMP task scheduling\n"
            "   overhead also rises.\n"
            "   Recommend 2x2 for small/medium scientific\n"
            "   sensors.\n"
            "\n\n"
            "2. Calibrating rlim:\n"
            "   As tile size shrinks, the maximum distance\n"
            "   between sub-frames drops. Scale rlim\n"
            "   proportionally to sqrt(tile_pixel_count).\n"
            "   Use -tileconf for per-tile rlim overrides\n"
            "   when tiles cover regions with different\n"
            "   dynamic ranges.\n"
            "\n\n"
            "3. Retrieval Window Sizing:\n"
            "   The default (1000) works for most data.\n"
            "   Increase to 5000-10000 for slowly-varying\n"
            "   signals with rare transitions. Decrease to\n"
            "   100-500 for rapidly-switching data where\n"
            "   old patterns are irrelevant.");
        printf("%sSEE ALSO%s\n",
               ANSI_BOLD_CYAN, ANSI_COLOR_RESET);
        print_see_also_option(
            "performance",
        ...
        print_see_also_option(
            "compression",
            "State space compression for tiled data");
        printf("\n");
        return 1;
    }
