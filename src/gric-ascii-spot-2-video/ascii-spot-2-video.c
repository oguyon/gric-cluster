...
void print_help(const char *progname)
{
    ...
    printf("  Reads a text file containing 3D coordinates (v1, v2, v3) and generates a 2D Gaussian\n");
    printf("  spot. Output can be an MP4 video or an ImageStreamIO shared memory stream.\n\n");
    ...
    printf("    %s-noise%s %s<val>%s     Add Gaussian noise with stddev <val> (%sDefault:%s%s 0.0%s)\n",
...
int main(int argc, char *argv[])
{
    ...
    if (isio_mode)
    {
        ...
        if (ImageStreamIO_createIm(&stream_image, output_file, 2, dims, _DATATYPE_FLOAT, 1, 1, 1) !=
            0)
            return 1;
        struct timespec ts;
    ...
    while (!stop_requested)
    {
        ...
        if (isio_mode)
        {
            ...
            double cx = (v1 + 1.5) / 3.0 * size, cy = (1.0 - (v2 + 1.5) / 3.0) * size,
                   sigma = size * alpha * (v3 + 1.5) / 2.0, ts2 = 2.0 * sigma * sigma;
            int r = (int)ceil(4.0 * sigma), mx = (int)cx - r, Mx = (int)cx + r, my = (int)cy - r,
                My = (int)cy + r;
            if (mx < 0)
                mx = 0;
            if (Mx >= size)
