/**
 * @file status_watch.c
 * @brief Interactive TUI dashboard event loop and metric monitors for gric-status.
 */
...
static void tui_draw_panel_info(
...
static void tui_draw_panel_framestats(
    const GricClusterShmStatus *status,
    int                         w_left,
    int                         w_right,
    int                         h_top,
...
static void tui_draw_panel_clusterstats(
    const GricClusterShmStatus *status,
    int                         w_left,
    int                         h_top,
    int                         h_bottom,
    ...
    double                      inst_fps)
{
    ...
    double avg_dists = (processed > 0)
                       ? (double)status->framedist_calls / (double)processed
                       : 0.0;
    ...
    int start_col = 2 + (int)strlen(prog_lbl);
    ...
    int filled = (int)(pct * (double)bar_width / 100.0);
...
static void tui_draw_panel_resources(
    const GricClusterShmStatus *status,
    int                         w_left,
    int                         w_right,
    int                         h_top,
    ...
    double                      rate_hz)
{
    ...
    prev_calc_time = (double)ts.tv_sec + (double)ts.tv_nsec / 1e9;
    ...
    while (!sig_quit)
    {
        ...
        if (!is_paused)
        {
            ...
            {
                elapsed_sec = (double)(now_cpu.tv_sec - last_cpu_time.tv_sec) +
                              (double)(now_cpu.tv_nsec - last_cpu_time.tv_nsec) / 1000000000.0;
            }
            if (elapsed_sec < 0.01)
            {
                elapsed_sec = 0.01;
            ...
            if (is_pid_alive(p_pid))
            {
                ...
                if (get_process_cpu_ticks(p_pid, &utime, &stime) == 0)
                {
                    ...
                    {
                        cluster_cpu = 100.0 * ((ticks - last_cluster_ticks) / (double)clk_tck)
                                      / elapsed_sec;
                    }
...

            {
                ...
                if (get_process_cpu_ticks(getpid(), &utime, &stime) == 0)
                {
                    ...
                    {
                        status_cpu = 100.0 * ((ticks - last_status_ticks) / (double)clk_tck)
                                     / elapsed_sec;
                    }
            ...
            double current_calc_time = (double)current_ts.tv_sec + (double)current_ts.tv_nsec / 1e9;
            ...
            tui_draw_panel_info(status, w_left, h_top);
            tui_draw_panel_framestats(status, w_left, w_right, h_top, processed);
            tui_draw_panel_clusterstats(status, w_left, h_top, h_bottom, processed, inst_fps);
            tui_draw_panel_resources(status, w_left, w_right, h_top, h_bottom,
                                     cluster_cpu, status_cpu);
