...
void print_help(const char *progname) {
    printf("Usage: %s [options] <clustered_file> [output_file]\n", progname);
    printf("Description:\n");
    printf("  Visualizes the clustering results from a .clustered.txt file.\n");
    printf("  Generates an SVG (default) or PNG image showing clusters and anchors.\n\n");
    printf("Arguments:\n");
    printf("  <clustered_file>  Input file generated by image-cluster (e.g. output.clustered.txt).\n");
    printf("  [output_file]     Optional output filename. Defaults to input filename with .svg/.png extension.\n\n");
    printf("Options:\n");
    ...
    #endif
    printf("\n");
    printf("  -fs <size>        Set font size for text labels (default: 12.0).\n");
}
...
int main(int argc, char *argv[]) {
    ...
    if (!input_filename) {
        if (argc > 1) fprintf(stderr, "Error: Missing input file.\n");
        print_help(argv[0]);
        return 1;
    }
