...
## Test Environment
- **Data**: Synthetic 128-dimensional sequences generated via `gric-mktxtseq`.
...
**Command**:
```bash
./gric-mktxtseq 10000 bench_spiral_slow.txt 2Dspiral
./gric-cluster 0.1 bench_spiral_slow.txt
```
...
*   **Time**: ~6 ms
*   **Calls**: ~3.7k (~3.7 calls per frame). This validates the expectation that after finding
    1-2 close clusters, the rest are pruned efficiently.
...
*   **Time**: ~8 ms
*   **Calls**: ~2.6k. Ideally, if the sequence is perfectly predicted, calls per frame approaches 1.
...
| 4-Point Pruning (`-te4`) | ~11,811 ms | 605,013 | Calls drop by ~40%; higher logic overhead. |
| 5-Point Pruning (`-te5`) | ~98,744 ms | 559,962 | Calls drop by ~45%; highest logic overhead. |
